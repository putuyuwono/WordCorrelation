...
//		private final static DoubleWritable one = new DoubleWritable(1);
	...
	public static class CombineClass extends Reducer<Text, Text, Text, Text>{
		...
		public void reduce(Text key, Iterable<Text> values,
			...
			Text outVal = new Text(sum + "|" + numOfRecord);
	...
	public static class ReduceClass extends
		...
		public void reduce(Text key, Iterable<Text> values,
				Context context) throws IOException, InterruptedException {
			double sumOccurrence = 0, sumWeighting = 0, totRes = 0;
			int counter = 0;
			for (Text val : values) {
				String comVal = val.toString();
				String[] temp = comVal.split("\\|");
				String occurrence = temp[0];
				String weighting = temp[1];
				sumOccurrence += Double.valueOf(occurrence);
				sumWeighting += Double.valueOf(weighting);
				counter ++;
			}
			
			if(counter == 3 && !status){
				status = true;
				totWeight = sumWeighting;			
			}
			
			if(status){
				totRes = sumOccurrence / totWeight;
				result.set(String.valueOf(totRes));
	...
	public static void run(String inputDir, String outputDir) throws Exception{
		...
		job.setCombinerClass(LevelBasedCorrelationAnalyser.CombineClass.class);
